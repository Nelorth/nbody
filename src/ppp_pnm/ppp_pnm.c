...
/**
 * Reads an integer value (represented as decimal digits)
 * from a file; skips any comment lines (starting with '#')
...
/**
 * Loads the parameters from a PNM image file.
 */
...
/**
 * Reads image data from a PNM image in raw format.
 */
...
/**
 * A <part> function for 'ppp_pnm_read_part' which reads
...
/**
 * Reads a PNM image (PBM, PGM, PPM) from a file. Return
...
/**
 * Subroutine for 'ppp_pnm_read' that uses a generic <part> function for
    ...
    if (fd != NULL) {
        if (ppp_pnm_params(fd, kind, rows, columns, maxval) == 0) {
            if (*maxval < 256) {
                ...
                if (image != NULL) {
                    if (ppp_pnm_read_data(fd, *kind, *rows, *columns, *maxval,
                                          image, offset, length) == 0) {
                        fclose(fd);
                        return image;
                    }
                    free(image);
